...
<template>
  <UButton icon="i-heroicons-bars-3" size="xl" color="white" variant="ghost" @click="toggleMenu"
    class="md:hidden absolute left-0" aria-label="Ouvrir le menu" />

  <transition name="slide">
    <nav v-if="isMenuOpen" class="flex flex-col absolute transition top-0 w-full h-lvh z-100 left-0 gap-14 z-[1]"
      :class="{'bg-[#181818]': isDark, 'bg-white': !isDark}">
      <UButton icon="i-heroicons-x-mark" size="xl" color="white" variant="ghost" @click="toggleMenu"
        class="self-start mt-2 ml-2" aria-label="Fermer le menu" />
      ...
      <section class="flex flex-col items-center justify-center font-bold text-3xl text-center gap-16" id="main-links">
        ...
        <NuxtLink to="/produits" activeClass="active-link" class="w-max" aria-label="Voir la page Notre catalogue">
          Notre catalogue</NuxtLink>
        <NuxtLink to="/artists" activeClass="active-link" class="w-max" aria-label="Voir la page Artistes">
          Artistes</NuxtLink>
        <NuxtLink to="/magasins" activeClass="active-link" class="w-max" aria-label="Voir la page Nos magasins">Nos
          magasins</NuxtLink>
        <NuxtLink to="/blogs" activeClass="active-link" class="w-max" aria-label="Voir la page Notre blog">Notre blog
        </NuxtLink>
      </section>
      ...
      <section class="flex justify-between mt-auto mb-1 px-1">
        <ClientOnly>
          <UButton :icon="isDark ? 'i-heroicons-moon-20-solid' : 'i-heroicons-sun-20-solid'" color="gray"
            variant="ghost" aria-label="Changer le thème du site" @click="isDark = !isDark" />
          <template #fallback>
            <div class="w-8 h-8" />
          </template>
        </ClientOnly>
