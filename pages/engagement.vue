...
useSchemaOrg([
  getDefinedPage(
    `${window.location.origin}/engagement`,
    title,
    description
  ),
])

const toggleLists: Array<Engagement> = getEngagements();
</script>
...
<template>
  ...
  <div>
    <AppToggleList v-for="(list, index) in toggleLists" :key="index" :toggleTitle="list.title"
      :toggleDescription="list.descriptions" />
  </div>
</template>