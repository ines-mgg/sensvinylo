...
<template>
    <section class="flex flex-col items-center my-4 p-2">
        ...
        <div class="flex gap-2 my-2 md:mt-4">
            <button class="border dark:text-white dark:border-white p-2" @click="decreaseQuantity" aria-label="Réduire la quantité">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M5 12h14" />
                </svg>
            </button>
            <span class="p-2 md:text-xl">{{ quantity }}</span>
            <button class="border dark:text-white dark:border-white p-2" @click="increaseQuantity" aria-label="Augmenter la quantité">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                </svg>
            </button>
        </div>
        <button class="p-2 rounded-lg w-full border font-bold my-2 md:w-1/2 md:text-xl" aria-label="Ajouter le vinyle au panier">Ajouter au panier</button>
    </section>
