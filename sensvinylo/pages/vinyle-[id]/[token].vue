...
<template>
    <section class="flex flex-col items-center my-4 p-2">
        ...
        <div v-if="item?.status === 'Nouveauté'" class="bg-orange-500 w-44 text-center font-semibold -mt-2">{{ item?.status }}</div>
        <div v-else-if="item?.status === 'Promotion'" class="bg-yellow-400 w-44 text-center font-semibold -mt-2">{{ item?.status }}</div>
        <div v-else-if="item?.status === 'Occasion'" class="bg-red-600 w-44 text-center font-semibold -mt-2">{{ item?.status }}</div>
        <span class="text-xl font-bold text-center">{{ artistItem?.name }} - {{ item?.title }}</span>
        <span class="text-xs font-bold text-center italic">{{ item?.year }} - {{ genreItem?.name }}</span>
        <div v-if="item?.status === 'Promotion' && item?.old_price !== ''" class="flex gap-1 font-semibold">
            <span class="line-through">{{ item?.old_price }}€</span>
            <span class="text-red-500">{{ item?.price }}€</span>
        </div>
        <div v-else class="font-semibold">{{ item?.price }}€</div>
        <div class="flex gap-2 my-2">
            <button class="border dark:text-white dark:border-white p-2 ml-2" @click="decreaseQuantity">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M5 12h14" />
                </svg>
            </button>
            <span class="p-2">{{ quantity }}</span>
            <button class="border dark:text-white dark:border-white p-2 ml-2" @click="increaseQuantity">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                </svg>
            </button>
        </div>
        <button class="p-2 rounded-lg w-full border font-bold my-2">Ajouter au panier</button>
    </section>
    <section class="flex flex-col items-center my-4 p-2">
